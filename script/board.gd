...
func gen_board():
   ...
   for r in range(total_size):
      ...
      for c in range(total_size):
         ...
         if (value == total_size*total_size):
            board[r].append(0)
            empty = Vector2(c, r)
         else:
...
func is_board_solved():
   ...
   for r in range(total_size):
      for c in range(total_size):
         if (board[r][c] != count):
            if r == c and c == total_size - 1 and board[r][c] == 0:
               return true
            else:
               return false
         count += 1
...
func scramble_board():
   ...
   var temp_flat_board = []
   for i in range(total_size*total_size - 1, -1, -1):
   ...
   for r in range(total_size):
      for c in range(total_size):
         board[r][c] = temp_flat_board[r*total_size + c]
         if board[r][c] != 0:
...
func reset_board():
   ...
   for r in range(total_size):
      board.append(([]))
      for c in range(total_size):
         board[r].append(r*total_size + c + 1)
         if r*total_size + c + 1 == total_size * total_size:
            board[r][c] = 0
         else:
            set_tile_position(r, c, board[r][c])
   empty = value_to_grid(0)
